...
- Use `recommend_providers` when the user asks which provider is best for a use case.
- Use `recommend_stack` when the user asks for a multi-part stack such as hosting + auth + postgres + email.
- Use `get_provider_detail` before making a strong recommendation about one exact provider.
...
- If the DB cannot confirm a requested feature from structured fields, say so and return a shortlist table instead of claiming one hard winner.
...
- Keep the winner summary above the table, not inside it.
...
3. Pull `get_provider_detail` on the top candidate only when the user needs a stronger answer on one exact provider.
4. Summarize the winner, the strongest alternative, and the main tradeoff.
...
| --- | --- | --- | --- | --- |
| Vercel | [Official](https://vercel.com) | $2 | Explicit MCP support | Best MCP-specific fit |
