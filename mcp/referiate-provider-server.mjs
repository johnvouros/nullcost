...
function getDefaultBaseUrl() {
  return process.env.REFERIATE_API_BASE_URL || process.env.REFERIATE_BASE_URL || DEFAULT_BASE_URL;
}
...
const CATEGORY_RULES = [
  ...
  {
    category: "ai",
    subcategories: ["inference_api", "gpu_compute"],
    patterns: [/\binference\b/, /\bllm\b/, /\bmodel(?:s)?\b/, /\bgpu\b/, /\bembedding(?:s)?\b/, /\btext generation\b/, /\bspeech\b/],
  },
  {
    category: "email",
...
const STACK_SLOT_RULES = {
  hosting: {
    ...
    promptSuffix: "hosting deployment app platform next.js runtime",
  },
  auth: {
    key: "auth",
    title: "Auth",
...
function parseNaturalLanguageIntent(query, context = "") {
  ...
  const explicitHostingIntent = /\bhost(?:ing|ed)?\b|\bdeploy(?:ing|ment|ments|s)?\b|\bapp platform\b|\bweb host(?:ing)?\b/.test(normalizedText);
  ...
  const contentTerms = uniqueValues(extractContentTerms(fullText));
  ...
  const features = {
    ...
    needApi: /\b(api|sdk|rest|http)\b/.test(normalizedText),
...
function rankCandidatePool(providers, parsedIntent, filters = {}, limit = DEFAULT_LIMIT) {
  const rankedCandidates = providers
    .filter((provider) => {
      ...
      if (filters.slotKey && !providerFitsStackSlot(provider, filters.slotKey)) {
        return false;
      }
  ...
  const recommendationPool =
    matchedCandidates.length >= minimumPool ? matchedCandidates : rankedCandidates;
...
server.registerTool(
  "search_providers",
  {
    description: "Search the Nullcost provider catalog by text query and optional category filters.",
    inputSchema: {
    ...
    if (!response.ok) {
      ...
      return {
        content: [
          {
            type: "text",
            text: `Nullcost provider search failed\n${message}`,
          },
        ],
        structuredContent: {
...
server.registerTool(
  "recommend_providers",
  {
    ...
    inputSchema: {
      ...
      needMcp: z.boolean().optional().describe("Prefer providers with explicit MCP availability."),
...
server.registerTool(
  "get_provider_detail",
  {
    description: "Fetch a single provider record by slug or name from the catalog API.",
    inputSchema: {
    ...
    if (!response.ok) {
      ...
      return {
        content: [
          {
            type: "text",
            text: `Nullcost provider detail failed\n${message}`,
          },
        ],
        structuredContent: {
